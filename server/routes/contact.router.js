...
router.get("/", (req, res, next) => {
    contact.find()
        ...
        .catch(err => {
            res.status(500).json({ message: "contact could not fetched!" })
        })
})
...
router.put("/:id", (req, res, next) => {
    contact.updateOne({ _id: req.params.id}, {
        ...
        .catch(err => {
            res.status(500).json({ message: "contact could not updated!", err: err })
        })
})
...
router.delete("/:id", (req, res, next) => {
    contact.deleteOne({ _id: req.params.id }).exec()
        ...
        .catch(err => {
            res.status(500).json({ message: "contact could not deleted!", err: err })
        })
})
